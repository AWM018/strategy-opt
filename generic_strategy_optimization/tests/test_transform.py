...
1513932000  641.64  653.93  653.19  651.99  472.897212
    ...
    assert len(df) == 5
    assert (df.index == pd.Int64Index([1513930800, 1513931100, 1513931400,
                                       1513931700, 1513932000])).all()
    assert_frame_equal(
        df,
        pd.DataFrame(
            [
...
def test_fine_downsample_works_with_9row_candles(candles_5m_40rows):
    """
Input:
               low    high    open   close      volume
ts
...
1513932300  641.64  667.97  650.26  664.46  742.716141
    ...
    assert len(df) == 6
    assert (df.index == pd.Int64Index([1513930800, 1513931100, 1513931400,
                                       1513931700, 1513932000, 1513932300])).all()
    assert_frame_equal(
        df,
        pd.DataFrame(
            [
...
def test_fine_downsample_works_with_10row_candles(candles_5m_40rows):
    """
Input:
               low    high    open   close      volume
ts
...
1513932600  646.55  671.93  647.67  669.00  813.722586
    ...
    assert len(df) == 7
    assert (df.index == pd.Int64Index([1513930800, 1513931100, 1513931400,
                                       1513931700, 1513932000, 1513932300,
                                       1513932600])).all()
    assert_frame_equal(
        df,
        pd.DataFrame(
...
def test_fine_downsample_works_with_11row_candles(candles_5m_40rows):
    """
Input:
               low    high    open   close      volume
ts
...
1513931100  631.00  635.68  635.24  634.21   74.753693   |--+
1513931400  633.58  653.76  633.58  653.36  128.697284   |  |--+
1513931700  650.22  664.66  653.42  654.77  189.076460   v  |  |--+
1513932000  650.26  653.93  653.19  650.26   85.447041      v  |  |--+
1513932300  641.64  651.06  650.26  647.68  169.788925         v  |  |--+
1513932600  646.55  653.74  647.67  650.00  103.725013            v  |  |--+
1513932900  649.90  653.68  649.99  651.99  113.936234               v  |  |--+
1513933200  652.00  667.97  652.33  664.46  355.265970                  v  |  |
1513933500  664.56  671.93  667.77  669.00  240.795369                     v  |
1513933800  657.33  669.62  669.12  666.19   96.332669                        v
